...
public class BoardImpl implements Board {
    ...
    public Iterable<Board> neighbors() {
        ...
        if (empty.j > 0) {
            neighbours.add(new BoardImpl(tiles).move(empty, getTile(empty.i, empty.j - 1)));
        }
        if (empty.j < size) {
            neighbours.add(new BoardImpl(tiles).move(empty, getTile(empty.i, empty.j + 1)));
        }
        if (empty.i > 0) {
            neighbours.add(new BoardImpl(tiles).move(empty, getTile(empty.i - 1, empty.j)));
        }
        if (empty.i < size) {
            neighbours.add(new BoardImpl(tiles).move(empty, getTile(empty.i + 1, empty.j)));
        }
    ...
    public Board move(Tile tile1, Tile tile2) {
